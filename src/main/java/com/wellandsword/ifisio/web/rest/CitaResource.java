...
public class CitaResource {
	 ...
	 *         body the new cita, or with status {@code 400 (Bad Request)} if the
	 ...
	 *         the updated cita, or with status {@code 400 (Bad Request)} if the
	 *         cita is not valid, or with status {@code 500 (Internal Server Error)}
	 *         if the cita couldn't be updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/citas")
	...
	public ResponseEntity<List<Cita>> getCitaCliente(@PathVariable Long id) {
		...
		List<Cita> citas = citaRepository.findByTratamientoClienteNumDocClienteId(id);
