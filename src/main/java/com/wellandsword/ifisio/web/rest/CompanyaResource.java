...
public class CompanyaResource {
	 ...
	 *         body the new companya, or with status {@code 400 (Bad Request)} if
	 *         the companya has already an ID.
	...
	public ResponseEntity<Companya> createCompanya(@RequestBody Companya companya) throws URISyntaxException {
		...
		return ResponseEntity
				.created(new URI("/api/companyas/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated companya, or with status {@code 400 (Bad Request)} if the
	 *         companya is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the companya couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<Companya> updateCompanya(@RequestBody Companya companya) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(
				HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, companya.getId().toString()))
				.body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the companya, or with status {@code 404 (Not Found)}.
	 */
